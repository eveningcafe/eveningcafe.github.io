...
function toTextString(text) {
    return text.reduce((result, textItem) => {
        ...
        if (textItem.annotations.bold === true) {
            return `${result}**${textItem.text.content}**`
        }
        if (textItem.annotations.italic === true) {
            return `${result}*${textItem.text.content}*`
        }
...
async function processBlocks(pageContent, textPrefix = "") {
    ...
    for (const block of pageContent.results) {
        ...
        if (block.type === "child_page") {
            ...
            dest = path.normalize(path.join(__dirname, "..", "static", "photos", "from-notion"))
            image_name = await downloadFile(block.image.file.url, dest)
            text = `${text}${textPrefix}\{\{\<\ figure src="/photos/from-notion/${image_name}" \>\}\}\n\n`
        } else if (block.type === "divider") {
            text = `${text}---\n`
        } else if (block.type === "quote") {
            text = `${text}${textPrefix}> ${toTextString(block.quote.text)}\n>\n\n`
        } else if (block.type === "embed" && block.embed.url.includes("spotify")) {
            pathname = new URL(block.embed.url).pathname;
            id = pathname.split('/')[2]
            text = `${text}${textPrefix}\{\{\< spotify type=\"track\" id=\"${id}" width=\"100%\" height=\"250\" \>\}\}\n\n`
            //{{< spotify type="track" id="1jw992uwXhqJJ0H42ucTRL" width="100%" height="250" >}}
        } else if (block.type === "numbered_list_item"){
            text = `${text}${textPrefix}${toTextString(block.numbered_list_item.text)}\n`
        }
        else {
            console.log("=====> Unhandled block: ", JSON.stringify(block, null, 2))
        }
    }
    if (metas.includes('showDate: false') === false) {
        metas.push(`showDate: true`)
    }
    if (metas.includes('comments: false') === false) {
        metas.push(`comments: true`)
    }
    return [text, metas]
}
